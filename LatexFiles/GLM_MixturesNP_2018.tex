...
For the case of a zero-inflated Poisson model,  it's sample space will further partition $\Omega_i$ such that $\Omega_i = \Omega^{B}_i \cap \Omega^{P}_i  $, where $\Omega^{B}_i$ is the sample space of excess zeros emanating from a Bernoulli model, and $\Omega^{P}_i$ is the sample space relating to a Poisson model. This allows for the construction of both pure Poisson, and zero-inflated Poisson mixtures to exist on $\Omega$.   }
