...
\textcolor{blue}{
For each particular set $\Omega_i$, a pure Poisson or a zero-inflated Poisson model exists. In the case of a Poisson model for group $i$, it's sample space $\Omega^P_i$, coincides with the sample space $\Omega_i$ such that  $\Omega_i = \Omega^P_i$.  
For the case of a zero-inflated Poisson model for group $i$,  it's sample space will coincide with the sample space $\Omega_i$ such that $\Omega_i = \Omega^{B}_i \cap \Omega^{P}_i  $, where $\Omega^{B}_i$ is the sample space of excess zeros emanating from a Bernoulli model, and $\Omega^{P}_i$ is the sample space relating to a Poisson model. This allows for the construction of both Poisson models, and zero-inflated Poisson models to exist on sample space $\Omega$.   }
