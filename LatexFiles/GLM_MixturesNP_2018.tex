...
g_p(\mathbb{E}\left[Y_{ClaimNb}|\bm{X}\right]) & = 
  \beta_{0} +  \beta_{Density} X_{Density}+ \beta_{Exposure} X_{Exposure}+ \beta_{Power} X_{Power}  \label{poissonReg} \\
g_z(\mathbb{E}\left[Y_{ClaimNb}|\bm{X}\right])& = \beta_0 + \beta_{Exposure} X_{Exposure} +  \beta_{CarAge} X_{CarAge} \label{zeroReg}
\end{align}
...
\begin{figure}[!ht]
\begin{center}
%\includegraphics[scale=0.60]{epsFreqZero.eps}
\end{center}
...
significance codes are the same as in Table \ref{severity_coef_table}. In each cluster we can see that the majority of the coefficients are significant. Specifically for the coefficients pertaining to the the Bernoulli zero-count models.  Furthermore, Cluster~1 had no evidence of zero-inflation by using the likelihood ratio test defined in \eqref{LRTest}, thus only a Poisson model was estimated. \textcolor{red}{[Reword previous sentence.]} In summary, we see that the GCWM can account for zero-inflated data in pricing.
...
We can observe that the MSEs for most of the models and their corresponding coefficients are not calculated at all due to convergence failures and as such they are shown as $(\cdot)$. This is not surprising because Table 4 shows the accuracy of CWM is not good when attempting to model non-Gaussian predictors as Gaussian. \textcolor{red}{[Why are we talking about Tables 4, 5 and 6 here? Again, nicknames should be used.]}
