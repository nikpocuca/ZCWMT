...
Where the sample space $\Omega^P$ is partitioned into  $G$ non-overlapping sets.
 ...
 \begin{align*}
  \Omega = & \quad \Omega^B \cup \Omega^P  =  \left(  \bigcup_{l=1}^{M\leq G} \Omega^B_l \right) \bigcup \left( \bigcup_{j=1}^G \Omega^P_j  \right) = \bigcup_{k \leq G} \left(\Omega_k^B \cup \Omega_k^P \right) :=  \bigcup_{k \leq G} \Omega_k^Z,
 \end{align*}
