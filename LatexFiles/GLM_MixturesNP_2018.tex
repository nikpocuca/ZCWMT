...
In this paper, for insurance purposes we propose two important extensions to the generalized cluster-weighted models (GCWM). First, we extend GCWM to allow for modeling non-Gaussian distribution of the continuous covariates as they are frequently found in insurance practice. Secondly, we introduce a zero-inflated extension of GCWM for modeling insurance claims data with excess zeros. Additionally, we give two expectation-optimization (EM) algorithms for parameter estimation in proposed extensions. Furthermore, an appropriate simulation study shows that both extended models perform well for different settings in contrast to the existing mixture-based approaches. Finally, a real data set based on French auto-mobile policies is used to illustrate the application of the proposed extensions.
...
For a zero-inflated model, the EM-algorithm follows a similar procedure as above to optimize the conditional density given in \eqref{ziGCWM}.  Specifically, the log-likelihood function of $\psi_k$ and $\lambda_k$ is expressed as
\begin{equation*}\begin{split}
...
where $\bm{\zz}_k := \left[\zz_{1k}, ..., \zz_{nk} \right]$ is a realization of $\bm{\zZ}_k  := \left[\zZ_{1k}, ..., \zZ_{nk} \right]$.
 ...
 Note that $l_c(\psi_k,\lambda_k|y,\bm{x},\bm{\zz}_k)$ separates allowing the maximization of  $l_c(\psi_k; y,\bm{x},\bm{\zz}_k) $ and $l_c(\lambda_k; y,\bm{x},\bm{\zz}_k) $ separately for parameters $\psi_k$ and $\lambda_k$. With the EM algorithm, maximization of parameters are done iteratively between estimating $\zZ_{ik}$ with its expectation under current estimates for $\lambda_k$ and $\psi_k$ (E-Step), and then maximizing the conditional expectation of the complete-data log-likelihood (M-Step). 
...
Given the log-normal assumption, the result of the transformation is a better AIC and BIC for GCWM over CWM.Table~\ref{comparingCWM_models} shows a considerable difference in BIC and AIC comparing CWM and GCWM. The five component CWM with a BIC of $281,680$ is significantly higher than the four component GCWM with a considerably lower BIC of $88,564$.
...
Overall GCWM outperforms CWM in both accuracy and MSE. 
\begin{table}[h!]
\centering
