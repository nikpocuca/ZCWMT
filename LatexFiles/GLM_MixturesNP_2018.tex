...
Where the sample space $\Omega^P$ is partitioned into  $G$ non-overlapping sets.
 ...
 \begin{align*}
  \Omega = & \quad \Omega^B \cup \Omega^P  =  \left(  \bigcup_{l=1}^{M\leq G} \Omega^B_l \right) \bigcup \left( \bigcup_{j=1}^G \Omega^P_j  \right) = \bigcup_{k \in \{ 1, ..., G  \}} \left(\Omega_k^B \cup \Omega_k^P \right) :=  \bigcup_{k \in \{ 1, ..., G  \} } \Omega_k^Z,
 \end{align*}
...
g(\mathbb{E}\left[Y_{LossAmount}|\bm{X}\right]) = 
 & \beta_{0} +  \beta_{Density} X_{Density}+ \beta_{Car Age} X_{Car Age}+ \beta_{Driver Age} X_{Driver Age} + \nonumber \\ &  \beta_{Region}X_{Region} + \beta_{Power}X_{Power} + \beta_{Gas} X_{Gas}+ \epsilon, \quad \epsilon \sim \mathcal{N}(0,\sigma^2). \label{regressionModel}
\end{align}
