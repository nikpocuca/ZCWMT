...
g(\mathbb{E}\left[Y_{LossAmount}|\bm{x}\right]) = 
 & \beta_{j0} +  \beta_{jDensity}x_{Density}+ \beta_{jCar Age} x_{Car Age}+ \beta_{jDriver Age} x_{Driver Age} + \nonumber \\ &  \beta_{jRegion} x_{Region} + \beta_{jPower} x_{Power} + \beta_{jGas} x_{Gas}. \label{regressionModel} \\
 := & \quad \bm{\xTilda} \bm{\beta}_j^{'}
\end{align}
...
g_p(\mathbb{E}\left[Y_{ClaimNb}|\bm{x}\right]) & = 
  \beta_{j0} +  \beta_{jDensity}x_{Density}+ \beta_{jExposure}  x_{Exposure}+ \beta_{jPower}x_{Power} \label{poissonReg}\\
 & =: \bm{\xTilda} \bm{\beta}_j^{'},  \nonumber  \\
g_z(\mathbb{E}\left[Y_{ClaimNb}|\bm{x}\right])& = \bar{\beta}_{j0} + \bar{\beta}_{jExposure} x_{Exposure} +  \bar{\beta}_{jCarAge} x_{CarAge} \label{zeroReg} \\
 & =: \bm{\xTilda} \bar{\bm{\beta}}_j^{'}, \nonumber
\end{align}
